...
saveBtn.addEventListener('click', async () => {
    ...
    message.className = '';
    message.textContent = 'A guardar imagem...';
    ...
    try {
        // Converte a imagem do preview para Blob
        const blob = await (await fetch(preview.src)).blob();
        const formData = new FormData();
        formData.append('cropped_image', blob, 'foto_perfil.jpg');

        const response = await fetch('/usuarios/foto/', {
            method: 'POST',
            body: formData,
            headers: {
        ...
        if (contentType && contentType.includes("application/json")) {
            ...
            if (result.success) {
                ...
                // Redireciona após sucesso para ver o perfil atualizado
                setTimeout(() => window.location.href = '/usuarios/foto/', 1500);
            } else {
                message.className = 'error';
                message.textContent = result.message || 'Erro ao guardar.';
            }
        } else {
            // Se o servidor enviou HTML (erro 403, 404 ou 500 não tratado)
            console.error("Erro do Servidor (HTML):", await response.text());
            message.className = 'error';
            message.textContent = 'O servidor respondeu com um erro inesperado.';
        }

    } catch (err) {
        console.error("Erro na ligação fetch:", err);
        message.className = 'error';
        message.textContent = 'Erro de ligação ao servidor.';
    } finally {
